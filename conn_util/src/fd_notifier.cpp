...
FdNotifier::FdNotifier() {
  ...
  if (!SetFdNonBlocking(read_fd_) || !SetFdNonBlocking(write_fd_)) {
    close(read_fd_);
    close(write_fd_);
    read_fd_ = -1;
    write_fd_ = -1;
...
bool FdNotifier::valid() const {
  return read_fd_ >= 0 && write_fd_ >= 0;
}

int FdNotifier::readFd() const {
  return read_fd_;
}
...
Status FdNotifier::notify() {
  ...
  const char byte = 'x';
  for (;;) {
    const ssize_t n = write(write_fd_, &byte, 1);
    if (n == 1) {
      return Status::Ok();
    }
    if (n < 0 && errno == EINTR) {
    ...
    if (n == 0) {
      return Status::IoError("zero-byte notifier write");
    }
